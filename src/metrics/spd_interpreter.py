...
class Conv1dComponent(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, groups, C, bias_tensor, stride=1, padding=0, dilation=1):
        super().__init__()
        self.C = C
    ...
    def forward(self, x):
        ...
        W_batch = W_batch.reshape(batch_size * self.out_channels, self.in_channels // self.groups, self.kernel_size)
...
def _patched_create(self, target_module_patterns, C):
        ...
        for pattern in target_module_patterns:
            if fnmatch.fnmatch(name, pattern):
                ...
                if isinstance(module, nn.Linear):
                    ...
                    raise ValueError(f"Module '{name}' matched pattern '{pattern}' but type {type(module)} is not supported.")
                break
    ...
    if not components:
        raise ValueError(f"No modules found matching target_module_patterns: {target_module_patterns}")
...
def _patched_calc_causal_importances(
    pre_weight_acts, As, gates, detach_inputs=False
):
    causal_importances = {}
    ...
    for param_name in pre_weight_acts:
        ...
        if not acts.dtype.is_floating_point:
            ...
            if acts.dim() == 3 and "conv1d" in param_name:
                ...
                component_act = einops.einsum(acts_unfolded, As[param_name], "b c k s, k C -> b s C")
        ...
        if isinstance(gates[param_name], component_utils_module.GateMLP):
            ...
            causal_importances_upper_leaky[param_name] = component_act * gates[param_name](gate_input)
...
def _patched_calc_faithfulness_loss(components, target_model, n_params, device):
    ...
    for name in component_params:
        faithfulness_loss = faithfulness_loss + ((target_params[name] - component_params[name]) ** 2).sum()
