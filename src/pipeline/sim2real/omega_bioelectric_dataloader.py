...
class OmegaBioelectricLoader:
    def __init__(self, sample_rate_hz=500, crash_minute=10):
        ...
        print(f"[INIT] Omega Dataloader targeting proxy: {self.source_url} at {self.sample_rate_hz}Hz")
    ...
    def apply_hardware_physics(self, pure_voltage, time_minutes):
        ...
        baseline_jitter = np.where(is_crashing, np.random.normal(0, 0.25, len(pure_voltage)), 
