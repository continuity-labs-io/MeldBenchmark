...
class AOLLSMDataset(Dataset):
    def __init__(self, data_dir, num_frames=199, crop_size=(128, 128, 128)):
        ...
        has_direct_stacks = any("stack0000" in f for f in os.listdir(data_dir)) or \
                            os.path.isdir(os.path.join(data_dir, "stack0000"))
        ...
        if has_direct_stacks:
            ...
            for item in sorted(os.listdir(data_dir)):
                ...
                if os.path.isdir(sub_path):
                    has_sub_stacks = any("stack0000" in f for f in os.listdir(sub_path)) or \
                                     os.path.isdir(os.path.join(sub_path, "stack0000"))
                    if has_sub_stacks:
        ...
        print(f"[INIT] AOLLSMDataset initialized with {len(self.sample_dirs)} sample(s) in: {data_dir}")
    ...
    def _crop_center(self, volume):
        ...
        if D < Cd or H < Ch or W < Cw:
            ...
            return volume[d_start:d_start+Cd, h_start:h_start+Ch, w_start:w_start+Cw].copy()
...
if __name__ == "__main__":
    ...
    if os.path.exists(raw_tiffs_dir):
        ...
        dataset_aollsm = AOLLSMDataset(data_dir=raw_tiffs_dir, num_frames=10, crop_size=(128, 128, 128))
        dataloader_aollsm = DataLoader(dataset_aollsm, batch_size=1, shuffle=False)
        for batch in dataloader_aollsm:
            print(f"Loaded Batch Shape [Batch, Time, Channels, Depth, Height, Width]: {batch.shape}")
