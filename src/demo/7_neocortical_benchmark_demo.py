...
def main():
    ...
    im1 = ax1.imshow(test_seq[0].detach().cpu().numpy().T, aspect='auto', cmap='viridis', origin='lower')
    ax1.axvline(x=EVENT_FRAME, color='yellow', linestyle='--', linewidth=2, label='EVENT_FRAME (Crash)')
