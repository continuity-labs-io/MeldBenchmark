...
def benchmark_data_08():
    ...
    transformer_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=6, batch_first=True).to(
        device
    )
    mamba_layer = Mamba(d_model=d_model, d_state=16, d_conv=4, expand=2).to(device)

    print(
        f"{'Temporal Context (Frames)':<28} | {'Transformer VRAM (O(L^2))':<28} | {'Mamba SSM VRAM (O(L))':<25}"
    )
    print("-" * 85)
