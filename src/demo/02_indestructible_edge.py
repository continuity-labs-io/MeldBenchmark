...
class WetLabDisasterSimulator:
    ...
    def generate_batch(self, batch_size, scenario="training", device="cpu"):
        t = torch.arange(self.seq_len, dtype=torch.float32, device=device).unsqueeze(0).unsqueeze(-1)
...
def plot_indestructible_dashboard(corrupt_seq, true_seq, pred_seq, drop_frame, output_dir):
    ...
    ax1.axvline(x=drop_frame, color='red', linestyle='--', linewidth=2, label='Catastrophic Sensor Dropout (NaN)')
    ax1.set_title("Analog Biological Layer (Massive Pump Artifact + Catastrophic Sensor Dropout)", color='white', fontweight='bold')
    ...
    ax2.plot(t_full, pred_signal, color='orange', linestyle='--', label='Model Imputed Prediction', linewidth=2)
    ax2.axvline(x=drop_frame, color='red', linestyle='--', linewidth=2)
    ax2.set_title("State-Space Recovery: Artifact Vetoed & Missing Sensor Imputed", color='white', fontweight='bold')
    ...
    ax3.axhline(y=0.1, color='lime', linestyle=':', linewidth=2, label='Theoretical Biological Noise Floor')
