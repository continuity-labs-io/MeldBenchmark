...
def plot_midterm_dashboard(time_axis, csd, ksm, lle, event_frame=None, filename="4_midterm_exam_dashboard.png"):
    ...
    ax3.plot(time_axis, lle, color="springgreen", linewidth=2.5, label="LLE (Attractor Basin Stability)")
    if event_frame is not None:
        ax3.axvline(x=time_axis[event_frame], color="white", linestyle="--", linewidth=2)
        ax3.axvspan(time_axis[max(0, event_frame-2)], time_axis[min(len(time_axis)-1, event_frame+2)], color="crimson", alpha=0.15)
    ax3.set_title("Chaos Signature: Local Lyapunov Exponent (LLE)", color="white", fontweight="bold")
...
def main():
    ...
    for step in range(MAX_TRAIN_STEPS):
        ...
        loss, loss_metrics = criterion(state_t, target_t_plus_1, pred_t_plus_1, reconstructed_t, delta_x)
        ...
        if (step + 1) % 5 == 0 or (step + 1) == 1:
            print(f"    Step {step+1:03d}/{MAX_TRAIN_STEPS} | Total Loss: {loss.item():.4f} | "
