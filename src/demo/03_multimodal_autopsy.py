...
def plot_autopsy_dashboard(raw_seq, relevance_map, trigger_frame, event_frame, output_dir):
    ...
    ax1.axvline(x=trigger_frame, color='yellow', linestyle=':', linewidth=2, label='RNA Stress Alarm (T=110)')
    ax1.axvline(x=event_frame, color='red', linestyle='--', linewidth=2, label='Structural Collapse (T=140)')
    ax1.set_title("Analog Biological Layer (114-D Trifecta Tensor)", color='white', fontweight='bold')
    ...
    im2 = ax2.imshow(relevance_map.T, aspect='auto', cmap='RdBu_r', origin='lower', vmin=-vmax, vmax=vmax)
    ax2.axvline(x=trigger_frame, color='yellow', linestyle=':', linewidth=2)
    ax2.axvline(x=event_frame, color='red', linestyle='--', linewidth=2)
    ax2.set_title("Digital Compute Layer: MambaLRPEpsilon Causal Attribution", color='white', fontweight='bold')
...
def main():
    ...
    engine = SpikeForecaster(input_dim=114, d_model=256, d_state=64).to(device)
