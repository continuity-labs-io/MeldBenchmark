...
I need a PyTorch `Dataset` and `DataLoader` pipeline designed to ingest continuous high-density electrophysiology data from raw 3Brain `.brw` files. The goal is to simulate the data topology of a 1,024-channel MaxOne CMOS array streaming at 20kHz, outputting chunked sequence tensors ready for a continuous-time state-space model (like Mamba-2).
...
2. The `__init__` method should accept `brw_file_path`, `sequence_length` (default 10000), and `target_channels` (default 1024). Use `spikeinterface.extractors.read_3brain` to load the recording lazily. Extract the total number of frames and the sampling rate.
3. The `__len__` method should return the total number of non-overlapping chunks (total frames divided by sequence length).
