...
1. The "GEVI + LLSM Sensor Fusion Injector" to demonstrate multi-rate sensor fusion and its impact on thermodynamic metrics (CVI and DAB).
...
- **Processing:** 
  - Pass `latent_anomalous` through `mamba_engine` -> extract loss and dab metric.
  - Pass `latent_fused_anomalous` through `mamba_engine_fused` -> extract loss and dab metric.
  - Calculate CVI and DAB using `ThermodynamicMetrics` for BOTH `latent_anomalous[0].detach()` (Optics-only) AND `latent_fused_anomalous[0].detach()` (Fused). 
...
- **Plotting (Update `plot_metrics.png`):**
  ...
  - **ax1 (DAB) & ax2 (CVI):** Plot TWO lines per metric: Optics-Only (Cyan/Magenta for DAB/CVI, marker='o', linestyle='--', label='Optical-Only') and Fused (Yellow/Orange for DAB/CVI, marker='s', linewidth=3, label='Fused Multi-Modal').
