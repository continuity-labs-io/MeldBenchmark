Context: We are creating a master execution script to prototype the continuous-time Mamba-2 engine on raw 1,024-channel HD-MEA data. We have `brw_dataloader.py` which ingests raw Zenodo 3Brain data, the `StateSpaceEngine` (Mamba-2), `ThermodynamicMetrics` (PyDMD), and `MambaLRPEpsilon`.
...
2. **Burn-in Training:** Pass the chunked tensor `[1, 10000, 1024]` into the `StateSpaceEngine`. Optimize it using `MeldLoss` for 10 iterations to burn in the biological standing wave dynamics. Track the peak VRAM allocation using `HardwareMonitor`.
