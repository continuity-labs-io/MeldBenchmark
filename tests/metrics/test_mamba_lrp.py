...
def test_relevance_conservation_axiom():
    ...
    model = SpikeForecaster(input_dim=16, d_model=32, d_state=8).to(device)
    ...
    assert error < 1e-2, f"Relevance Conservation Axiom Violated! Expected: {expected_relevance}, Actual: {actual_relevance}, Error: {error}"
